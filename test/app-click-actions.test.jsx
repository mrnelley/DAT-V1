...
describe('clickable user actions', () => {
  ...
  it('creates a Weekly Tracker huddle and sends the weekly goals Teams card', async () => {
    ...
    global.fetch = async () => ({
      ...
      json: async () => ({
        card: {
          body: [{ text: 'Input your Weekly Tracker goals' }],
          actions: [{ title: 'Open Weekly Tracker' }],
        },
        delivery: {
    ...
    try {
      ...
      await waitFor(() => {
        ...
        expect(dispatch.card.actions.some((action) => action.title === 'Open Weekly Tracker')).to.equal(true);
      });
    } finally {
