...
async function setup(){
 ...
 const roster={members:[{userId:'admin',email:'admin@example.invalid',positionTitle:'Manager, Enterprise Initiatives',roles:['admin'],departments:['Finance'],active:true,confirmed:true,positions:[],readMetrics:null,writeMetrics:null,readWeekly:null,writeWeekly:null}],departments:['Finance','Property Management'],positions:[{id:'finance-position',title:'Director of Finance'}]};
 ...
 const store={session:async()=>({}),access:async()=>({admin:true,weekly:true,metrics:true,positionTitle:roster.members[0].positionTitle}),members:async()=>roster,targets:async()=>[],adminWorkspace:async()=>workspace,
  manageUser:async p=>{calls.push({type:'create',payload:p});const member={userId:'created',email:p.email,positionTitle:'Director of Finance',roles:[],departments:['Finance'],positions:p.positionIds,confirmed:false,active:true};roster.members.push(member);return {userId:'created',message:'Account created.'};},
...
test('command center includes the prior controls and creates a user with assigned roles',async()=>{
 const {dom,w,d,calls}=await setup();assert.equal(d.querySelectorAll('[data-admin-tab]').length,10);
 const form=d.querySelector('#create-user-form');form.elements.email.value='person@example.invalid';form.querySelector('[name="positionIds"]').checked=true;
 form.dispatchEvent(new w.Event('submit',{cancelable:true}));await tick();assert.equal(calls[0].type,'create');assert.deepEqual([...calls[0].payload.positionIds],['finance-position']);assert.ok(calls[0].payload.requestId);assert.match(d.querySelector('#create-user-result').textContent,/created/);assert.equal(d.querySelector('#admin-user').value,'created');assert.ok(d.querySelector('#send-invitation'));dom.window.close();
});
test('uncertain create responses preserve the request ID and entered values for retry',async()=>{
 const {dom,w,d,store}=await setup(),seen=[];store.manageUser=async p=>{seen.push(p.requestId);throw new Error('Network response unavailable');};const form=d.querySelector('#create-user-form');form.elements.email.value='person@example.invalid';form.querySelector('[name="positionIds"]').checked=true;
 form.dispatchEvent(new w.Event('submit',{cancelable:true}));await tick();form.dispatchEvent(new w.Event('submit',{cancelable:true}));await tick();assert.equal(seen.length,2);assert.equal(seen[0],seen[1]);assert.equal(form.elements.email.value,'person@example.invalid');assert.match(d.querySelector('#create-user-result').textContent,/Network response/);dom.window.close();
});
